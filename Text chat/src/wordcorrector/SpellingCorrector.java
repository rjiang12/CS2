...
public class SpellingCorrector {
    ...
    public static int editDistance(String a, String b) {
        return editDistance(a, b, a.length(), b.length(), new Integer[a.length()][b.length()]);
    }

    private static int editDistance(String a, String b, int m, int n, Integer[][] memoTable) {
        // base case
        if(m == 0) {
            return n;
        }
        ...
        // make life easier, type fewer - 1s.
        m--;
        n--;
        // check against memoization table
        if(memoTable[m][n] != null) {
            return memoTable[m][n];
        }
        // last letters are the same, lob off last characters
        if(a.charAt(m) == b.charAt(n)) {
            memoTable[m][n] = editDistance(a, b, m, n, memoTable);
            return memoTable[m][n];
        }
        // last letters different.
        int insertion = editDistance(a, b, m + 1, n, memoTable) + 1;
        int deletion = editDistance(a, b, m, n + 1, memoTable) + 1;
        int replacement = editDistance(a, b, m, n, memoTable) + 1;
        memoTable[m][n] = Math.min(Math.min(insertion, deletion), replacement);
        return memoTable[m][n];
    }
